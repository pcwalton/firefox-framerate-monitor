...
Monitor.prototype = {
    ...
    observe: function(subject, topic, data) {
        ...
        this._model.samples.push({ potential: potential, actual: actual });
        ...
        if (this.model && this.model.samples) {
            ...
            for (let x = 16; x < canvasWidth; x += 2) {
                if (sampleIndex >= 0 && sampleIndex < samples.length) {
                    ...
                    ctx.fillRect(x, y, 2, potentialHeight);

                    ctx.globalAlpha = 0.5;
                    ctx.fillStyle = "#000000";
                    let shadeHeight = Math.max(0,
                        Math.min(canvasHeight / 60 *
                                 (sample.potential - sample.actual),
                                 canvasHeight));
                    ctx.fillRect(x, y, 2, shadeHeight);
                }
