...
Model.prototype = {
    addSample: function(sample) {
        ...
        if (sample.potential < this.least)
            this.least = sample.potential;
    },
...
FramerateWindow.prototype = {
    ...
    sampleTaken: function(id, sample) {
        this.view.redraw();

        let model = this._getModelForWindowID(id);
        model.addSample(sample);

        this.potentialFPSDisplay.innerHTML =
            "Potential: " + Math.floor(sample.potential) + " fps";
        this.actualFPSDisplay.innerHTML =
            "Actual: " + Math.floor(sample.actual) + " fps";

        if (this._autoStopProfiler) {
            let profilingThreshold = parseInt(this.profilingThreshold.value);
            if (sample.potential < profilingThreshold)
                this._stopProfiler();
        }
    ...
    _getMonitorForWindowID: function(windowID) {
        ...
        if (!('_framerateMonitor' in window)) {
            ...
            window._framerateMonitor = new Monitor(this, windowID, window);
        }
